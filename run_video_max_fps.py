...
from scripts.test_filtered_cls import load_model, process
...
def classify_detections(boxes_moving, borders, names):
    ...
    for box in boxes_moving:
        ...
        boxes_info.append({
            "xywh": box.xywh[0].tolist(), "conf": box.conf[0].item(), 
...
def run_inference_and_draw(frame, model_seg, model_det, transform_seg, target_distances, device, num_ys=15):
    ...
    with torch.no_grad():
        with autocast(enabled=(device.type == 'cuda')):
            ...
            id_map = process(model_seg, input_tensor_seg, None, "segformer", output_size=(original_h, original_w))
            ...
            results = model_det.predict(frame, verbose=False) 
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="RailSafeNet Max Performance Video Processor.")
    ...
    model_det = YOLO(PATH_model_det)
    model_det.to(device)
    if hasattr(model_det, 'model'):
        model_det.model.eval()
    ...
    while cap.isOpened():
        ...
        fps_text = f"Processing FPS: {avg_fps:.2f}"
        cv2.putText(processed_frame, fps_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
        
        cv2.imshow('RailSafeNet Max Performance', processed_frame)
